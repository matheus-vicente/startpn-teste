...
import { EditFieldShipperUseCase } from "./edit-field-shipper";
...
let sut: EditFieldShipperUseCase;

describe("Edit Field Shipper", () => {
  beforeEach(() => {
    ...
    sut = new EditFieldShipperUseCase(inMemoryFieldsRepository);
  });

  it("should be able to edit a field shipper", async () => {
  ...
  it("should not be able to edit a field shipper with wrong id", async () => {
    await inMemoryFieldsRepository.create(
      makeFieldShipper(
        { name: "Custom Name" },
        new UniqueEntityId("custom-id"),
  ...
  it("should not be able to edit a field shipper if the name was already stored", async () => {
    await inMemoryFieldsRepository.create(
      makeFieldShipper(
        { name: "Custom Name 01" },
